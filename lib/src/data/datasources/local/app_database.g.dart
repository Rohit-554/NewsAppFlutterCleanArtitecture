    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `articles_table` (`id` INTEGER PRIMARY KEY AUTOINCREMENT, `source` TEXT NOT NULL, `author` TEXT, `title` TEXT NOT NULL, `description` TEXT, `url` TEXT NOT NULL, `urlToImage` TEXT, `publishedAt` TEXT NOT NULL, `content` TEXT)');
    ...
    this.changeListener,
        ...
        _articleInsertionAdapter = InsertionAdapter(
            ...
            (Article item) => <String, Object?>{
                  'id': item.id,
                  'source': _sourceClassConverter.encode(item.source),
                  'author': item.author,
                  'title': item.title,
  ...
  Future<List<Article>> getAllArticles() async {
    return _queryAdapter.queryList('SELECT * FROM articles_table',
        mapper: (Map<String, Object?> row) => Article(
            id: row['id'] as int?,
            source: _sourceClassConverter.decode(row['source'] as String),
...
final _sourceClassConverter = SourceClassConverter();
